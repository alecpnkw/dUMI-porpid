...
using PORPID, PyPlot, StatsBase, HypothesisTests,
    IterTools, DataFrames, CSV, DataFramesMeta
...
function filterCCSFamilies(most_likely_real_for_each_obs, path, index_to_tag, tag_counts, template_name)
    ...
    for (observed_index_local, tuple_local) in enumerate(most_likely_real_for_each_obs)
        ...
        prob = tuple_local[2]
        if tag_counts[tagSeq] < 5 # set fs thresh here
            prob = 0
        end
        ...
        p_val = 1
        if mean(averages[7:7+8])<mean(averages[18:20+18])
            p_val=pvalue(EqualVarianceTTest(averages[7:7+8],averages[18:20+18]))
        end

        if p_val < 1/(5*tag_counts[tagSeq]^2) && minimum(averages[7:7+8]) < mean(averages[18:20+18])/2
            prob = 0
        end
